...
////////////////////////////////////// BEGINNING OF E-STEP //////////////////////////////////////////
			...
			for (int im = 0; im < mcmc_chain_size ; ++im) //im = 1000 chain
			{
				...
				// masud: making real demand out of multipliers
				if (is_dev_demand) 
				{
					//tag:real demand	
					for (int i=0; i<dim; ++i) 
				...
				{
					//Debug 5 -2(see below): description
					float sm_tolerance1, sm_tolerance2; // masud: Jinduan used a tolerance of 0.9 and 1.1 
					sm_tolerance1 = 0.001;
					sm_tolerance2 = 1.9;

					for (int id = 0; id < dim; ++id) 
					{ //reject problematic demands
						if (cur_xd[id] < xd0[t*dim+id] * sm_tolerance1 || 
							cur_xd[id] > xd0[t*dim+id] * sm_tolerance2) 
						{
							if (t == 0 && im < 200) 
							{
								fprintf(fp162, "%3d %5.1f\t%5.1f",im, cur_xd[id], xd0[t*dim + id]);
								//printf("%3d %5.1f\t%5.1f", im, cur_xd[id], xd0[t*dim + id]);
								if (id == dim - 1) 
								{ 
									fprintf(fp162, "\n"); 
									//printf("\n");
							...
							has_wrong_demand = 1; //masud: its impact is in MH algorithm: go to ---> tag: MH_alg
							wrong_demand_count++;
							break;
						}
					}
				...
				*/
				...
				if (t == 6 && im == 0)
					smbool = true;
				//masud: getting hydraulic likelyhood
				sv->logL(&scada[t*n_ch], n_ch, &cur_ll,false,smbool);
				...
				//%%%%%%%%%%%%%%%%% DEBUG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				//masud: debug 1 -- I am adding if(t>48) to not calculate till it is 48 hrs. 
				// Originially jinduan had it from t = 0 onward
				//if(t>72)
				...
				//%%%%%%%%%%%%%%%%% DEBUG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				//masud: debug 5 -- ignoring VAR model likelihood by adding the line below:
				dm_ll = 0.0;
				//debug iv: masud:
					
				if (iter == 0 && t == 6)
					fprintf(fp163, "%6.1f,", cur_ll);

				cur_ll += dm_ll;
				if (iter == 0 && t == 6)
					fprintf(fp163, "%6.1f,%6.1f\n", dm_ll,cur_ll);
				...
				//Original line: //if (has_wrong_demand || cur_ll < last_ll && rng->Random() > exp(cur_ll - last_ll)) 
				//masud: Replacement of the original line above for debugging.
				if (poorer_likelihood && rejection_prob)	//masud: bad_likelihood is redundant??
				{
			...
			}//End of for (int im = 0; im < mcmc_chain_size ; ++im) //im = 1000 chain
			 //End of for (int im = 0; im < mcmc_chain_size ; ++im) //im = 1000 chain
			 //End of for (int im = 0; im < mcmc_chain_size ; ++im) //im = 1000 chain
...
///////////////////// End of MC Chain ///////////////////////////////////////////////////////////////////////
			...
			//masud:
			//DEBUG: ending program after estimation of the first hour.
			//if (t == 0)
				//goto SM_DEBUG_END;
			...
			//%%%%%%%%%%%%%%%%% DEBUG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			//masud: debug 6 -- adjusting prop_std for the next hours 10000 sim
			//double sm_check, sm_accepted;
			if (iter >= 0)
			{	
				double minimum_prop_std = 1;
				if (pct_acc < 25.0 && prop_std > minimum_prop_std)
				{
					prop_std /= 1.7;
					printf("(Std: %3.1f)\t", prop_std);
				}
				if (pct_acc > 45.0)
				{
					prop_std *= 1.7;
...
////////////////////////SM.-----------END OF E-Step---------------------------------------------------///////
		...
		double diff; // masud: difference of  VAR model?
		...
		//masud: the following three lines are if the VarModel_estimate() is commented out
			VarModel_Err vmerr = VarModel_Err::VARMODEL_OK;
			diff = 0; 
			mdll = 0;
		...
		//%%%%%%%%%%%%%%%%% DEBUG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
		// DEBUG 3: See above
		//Masud: debug 3 -- the following line was commented, and I am uncommenting it.
		//masud: updates initial guess vector with current demand
		...
		printf("Sum of changes in parameters:\n "
			"diff in mu %.2f, VAR likelihood: %.2f, Hyd Likelihood: %.2f, total l:%.2f\n", 
			diff, mdll, mhll, mdll+mhll); 
		//if (t_pct_acc > 60) {
			//prop_std *= 1.3;
		//} else if (t_pct_acc < 15) {
		//masud: debug 2 -- I commented the following line
		//	prop_std /= 1.5;	//masud: At every EM iteration Jinduan used to decrease prop_std by 1.5 
		//}
		//if (iter >= 5) {
