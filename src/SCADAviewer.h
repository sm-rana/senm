/* Demand-Hydraulic (DHM) Model of water distribution systems 
Copyright (C) 2015  Jinduan Chen <jinduan.uc@gmail.com>
...
/* class SCADAviewer and SCADAviewerInterationStyle provide dynamic
visualization of the SCADA systems and the water distribution system models*/
...
class SCADAviewerInterationStyle : public vtkInteractorStyleTrackballCamera {
	// iteration style for handling mouse events
protected:

	vtkPropPicker*		_pk;
	vtkCellPicker*		_pkCell;
	SCADAviewer*				_hl; // event handler
	CRITICAL_SECTION*	_propProtector;

public:
	enum Err { OK, NO_RENDERER, NO_EVENT_HANDLER };
	
	static SCADAviewerInterationStyle* New();
	vtkTypeMacro(SCADAviewerInterationStyle, vtkInteractorStyleTrackballCamera);

	// set the mouse event handler (updateSelection) and critical section for
	// protecting rendering
	Err init(SCADAviewer* handler_in, CRITICAL_SECTION* propProtector) ;
...
protected:
	...
	VisNetwork* _vnet; // network visualization
	Varima* _dmodel;  //demand model

	int _n; //number of water users, also dimensions of the unkown water demands
	...
	SCADAviewerInterationStyle *_sisPhi;
	...
	SCADAviewerInterationStyle *_sisS;
...
public:
	...
	SCADAviewer(): _dbcstr(NULL),_netfile(NULL),
	...
	Err updateSelection(SCADAviewerInterationStyle*, vtkPropAssembly*);
	Err updateSelection(SCADAviewerInterationStyle*, vtkMapper* selMp, vtkIdType cellid);
	Err run();
